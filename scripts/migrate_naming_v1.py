...
def migrate_duckdb_metadata(config: MigrationConfig) -> list[dict[str, Any]]:
    ...
    with duckdb.connect(str(config.duckdb_file)) as conn:
        ...
        if config.apply:
            ...
            if duckdb_table_exists(conn, "pipeline_checkpoints") and duckdb_column_exists(conn, "pipeline_checkpoints", "pipeline"):
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    mode.add_argument("--apply", action="store_true", help="Apply the migration. Without this flag the script is dry-run.")
    mode.add_argument("--dry-run", action="store_true", help="Plan the migration without writing data. This is the default.")
