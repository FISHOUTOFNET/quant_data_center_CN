...
class DataRegistry:
    ...
    def refresh_inventory(
        ...
        with self._lock:
            ...
            remaining = (
                current.loc[~current["dataset_id"].astype(str).isin(requested)] if not current.empty else current
            )
            ...
            pq.write_table(
                pa.Table.from_pandas(refreshed, schema=REGISTRY_INVENTORY_SCHEMA, preserve_index=False),
                self.inventory_path,
            )
            return refreshed
...
def _scan_dataset_dir(dataset_dir: Path, definition: DatasetDefinition) -> dict[str, Any]:
    parquet_files = (
        [path for path in dataset_dir.rglob("*.parquet") if path.is_file() and ".tmp.parquet" not in path.name]
