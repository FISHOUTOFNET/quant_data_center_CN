...
from src.pipeline.update_daily_metadata import _persist_lifecycle_rows
...
def _prepare_partial_baostock_cn_trading_calendar(
    ...
    store.write_dataset("baostock_cn_trading_calendar", fetched_baostock_cn_trading_calendar_df)
...
def _prepare_full_baostock_cn_trading_calendar(
    ...
    if baostock_cn_trading_calendar_covers_range(
        baostock_cn_trading_calendar_df, start_candidate_date, candidate_end_date
    ):
    ...
    fetched_baostock_cn_trading_calendar_df = provider.query_trade_dates()
    log_api_fetch(
        "baostock_cn_trading_calendar", "*", FULL_HISTORY_START_DATE, "latest", fetched_baostock_cn_trading_calendar_df
    )
    store.write_dataset("baostock_cn_trading_calendar", fetched_baostock_cn_trading_calendar_df)
    baostock_cn_trading_calendar_df = store.read_dataset("baostock_cn_trading_calendar")
    start_date, end_date = trading_range_bounds(
...
def _write_baostock_cn_trading_calendar_target(
    ...
    if fetched_baostock_cn_trading_calendar_df is None:
        ...
        baostock_cn_trading_calendar_path = store.write_dataset(
            "baostock_cn_trading_calendar", fetched_baostock_cn_trading_calendar_df
        ).primary_path
        baostock_cn_trading_calendar_df = store.read_dataset("baostock_cn_trading_calendar")
        if mode == "partial":
            start_date, end_date = _resolve_update_window(
                baostock_cn_trading_calendar_df, candidate_end_date, lookback_days
            )
        else:
            start_date, end_date = trading_range_bounds(
                baostock_cn_trading_calendar_df, start_candidate_date, candidate_end_date
        ...
        baostock_cn_trading_calendar_path = store.dataset_path("baostock_cn_trading_calendar")
...
def _try_resolve_update_window(
    ...
    if not baostock_cn_trading_calendar_covers_range(
        baostock_cn_trading_calendar_df, baostock_cn_trading_calendar_start_date, candidate_end_date
    ):
        return None
    try:
