"""Adapter for AkShare stock_info_sz_delist."""
...
from src.sources.akshare.core.symbols import clean_source_symbol, normalize_source_code
from src.storage.schema import AKSHARE_DELIST_SZ_SCHEMA, field_names
...
class DelistSzAdapter:
    symbol: str
    snapshot_date: str
    fetched_at: datetime

    endpoint: str = "stock_info_sz_delist"
    ...
    def call(self, ak_module: Any) -> object:
        return ak_module.stock_info_sz_delist(symbol=self.symbol)
    ...
    def normalize(self, source_df: pd.DataFrame) -> pd.DataFrame:
        ...
        selected = select_required_columns(source_df, AKSHARE_DELIST_SZ_FIELD_ALIASES, "akshare_cn_stock_delist_sz")
