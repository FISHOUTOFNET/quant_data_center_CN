...
from src.sources.baostock.client import BaostockClient
from src.sources.common.market_data import DailyBarRequest
...
class BaostockProvider:
    ...
    def __enter__(self) -> BaostockProvider:
        client = self._client_factory(
            max_attempts=int(self._config.get("pipeline.max_retries", 3)),
            timeout_seconds=float(self._config.get("api.baostock.timeout_seconds", 60)),
            max_rows_per_result=int(self._config.get("api.baostock.max_rows_per_result", 200000)),
        )
