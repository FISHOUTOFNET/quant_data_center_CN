...
如果本地没有 AkShare 清单且未显式传 `--code`，pipeline 会提示先运行 `update-akshare-spot` 或 `update-akshare-universe`。
...
旧 AkShare `code=sh.600000` 分区不会自动迁移。新代码不会再写旧格式。
