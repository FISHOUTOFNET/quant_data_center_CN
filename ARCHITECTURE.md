...
- 显式 `--period` 可重复传入，并且只允许用于 `report_disclosure` target。
- 巨潮/AkShare 对部分历史早期无数据期会抛出空数据列匹配异常；adapter 将该固定空历史期错误规范化为空 DataFrame，pipeline 写入空 schema 分区并记录 success。

周五至周日的 `scripts/run_update_daily.bat` 会在周末窗口内运行 `report_disclosure` partial 更新，并在最终统一 build DuckDB views 前完成。
