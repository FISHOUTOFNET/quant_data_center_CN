...
- AkShare：独立爬虫流程，保存估值、股本结构、预约披露时间和 A 股行情相关数据。按股票取数的数据集使用 6 位 `code`，预约披露时间按 `market + period` 一次性取数，不循环股票代码。
...
- `--period` 只适用于 `report_disclosure`，可重复传入，格式如 `2025年报`、`2026一季`。
...
│       └── modules/        # valuation、capital_structure、report_disclosure、daily_bar、spot_quote、delist
├── quality/                # 写入前验证
