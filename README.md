...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `--dataset` | `all` | 数据集：`daily_k_none`/`daily_k_qfq`/`daily_k_hfq`/`all`/`stock_basic`/`calendar` |
| `--start` | `1990-01-01` | 开始日期，格式 `YYYY-MM-DD`；非交易日时顺延到区间内首个交易日 |
| `--end` | 18:00 前为前一自然日，18:00 后为当天，并回退到最近交易日 | 结束日期，格式 `YYYY-MM-DD`；显式传入非交易日时回退到最近交易日 |
| `--code` | 最新 stock_basic 快照 | 股票代码，可重复指定多个 |
| `--universe` | 无 | 已弃用的股票池名称，读取 `config/universe.yaml`，仅用于兼容旧流程 |
| `--provider` | `config/settings.yaml` 中的 `api.provider` | 数据源名称；当前内置 `baostock` |
| `--resume/--no-resume` | `--resume` | 启用续传，跳过已成功的 checkpoint |
| `--force` | 否 | 忽略 checkpoint，强制重新拉取 |
| `--build-views/--no-build-views` | `--build-views` | 完成后构建 DuckDB 视图 |

**输出示例**：

```
daily_k_none sh.600000 status=success rows=82 path=data/parquet/daily_k_none/code=sh.600000/data.parquet
daily_k_qfq sh.600000 status=success rows=82 path=data/parquet/daily_k_qfq/code=sh.600000/data.parquet
daily_k_hfq sh.600000 status=success rows=82 path=data/parquet/daily_k_hfq/code=sh.600000/data.parquet
stock_basic * status=success rows=5000 path=data/parquet/stock_basic/data.parquet
calendar * status=success rows=365 path=data/parquet/calendar/data.parquet
```

### qdc update-daily

日常增量更新，使用回看窗口校验并合并覆盖历史数据。
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `--code` | 最新 stock_basic 快照中 type=1 且 status=1 的代码 | 股票代码，可重复指定多个 |
| `--universe` | 无 | 已弃用的股票池名称，读取 `config/universe.yaml`，仅用于兼容旧流程 |
...
4. 按 `lookback-days` 向前数交易日，拉取每个代码、每个复权数据集的回看窗口数据
...
| `--dataset` | 必填 | 数据集：`daily_k_none`/`daily_k_qfq`/`daily_k_hfq`/`daily_k_all` |
| `--provider` | `config/settings.yaml` 中的 `api.provider` | 数据源名称；当前内置 `baostock` |
| `--build-views/--no-build-views` | `--build-views` | 完成后构建 DuckDB 视图 |
...
- `qdc init-history`、`qdc update-daily`、`qdc repair` 都支持用 `--provider` 覆盖默认数据源
...
- `qdc init-history` 默认使用最新本地 `stock_basic` 快照中的全部非空 `code`
