...
def test_update_daily_prefilter_skips_code_when_requested_targets_are_checkpointed(
    ...
    factor_path = store.write_baostock_cn_stock_adjustment_factor(code, factors)
...
def test_update_daily_all_keeps_code_when_any_requested_daily_checkpoint_is_missing(
    ...
    factor_path = store.write_baostock_cn_stock_adjustment_factor(code, factors)
    qfq_path = store.write_baostock_daily_bars("baostock_cn_stock_daily_bar_qfq", code, daily_sample().assign(code=code, adjust_flag="1"))
    write_checkpoint(
        store,
        PIPELINE_UPDATE_DAILY,
        BAOSTOCK_CN_STOCK_ADJUSTMENT_FACTOR_DATASET,
    ...
    assert daily_datasets == {"baostock_cn_stock_daily_bar_unadjusted", "baostock_cn_stock_daily_bar_qfq", "baostock_cn_stock_daily_bar_hfq"}
...
def test_update_daily_resolves_weekend_end_to_previous_trading_day(
    ...
    store = ParquetStore(root=tmp_path)
    assert store.baostock_cn_stock_basic_path().exists()
