...
def test_derived_dataset_staging_area_only_allows_derived(tmp_path) -> None:
    store = ParquetStore(root=tmp_path)

    staging = create_derived_dataset_staging_area(store, "cn_stock_daily_bar")

    assert staging.staging_root.parent == tmp_path / "data" / "parquet" / ".staging"
    assert staging.staging_dataset_dir == staging.staging_root / "cn_stock_daily_bar"
    assert staging.staging_dataset_dir.is_dir()
    cleanup_derived_dataset_staging(staging)
    with pytest.raises(ValueError, match="non-derived"):
        create_derived_dataset_staging_area(store, "baostock_cn_stock_daily_bar_qfq")
