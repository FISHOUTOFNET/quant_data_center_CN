...
def _fake_provider_factory(
    ...
    factors = (
        baostock_cn_stock_adjustment_factor_df
        if baostock_cn_stock_adjustment_factor_df is not None
        else pd.DataFrame(
            [
                {
                    "code": "sh.600000",
    ...
    delays = dict(api_delays or {})

    def apply_delay(name: str) -> None:
