...
def test_update_daily_refetches_full_history_on_lookback_mismatch(
    ...
    for dataset, adjust_flag in adjust_flags.items():
        ...
        store.write_dataset(dataset, existing, {"code": "sh.600000"})
    ...
    assert [item["dataset"] for item in daily_records] == ["baostock_cn_stock_daily_bar_unadjusted"]
    assert [item["start_date"] for item in daily_records] == ["1990-01-01"]
    assert store.read_dataset("baostock_cn_stock_daily_bar_unadjusted", {"code": "sh.600000"}).loc[0, "close"] == 8.2
...
def test_update_daily_refetches_full_history_when_lookback_is_empty(
    ...
    daily_records = [item for item in records if item["dataset"].startswith("baostock_cn_stock_daily_bar_")]
    assert [item["dataset"] for item in daily_records] == ["baostock_cn_stock_daily_bar_unadjusted"]
    assert [item["row_count"] for item in daily_records] == [2]
...
def test_update_daily_does_not_refetch_when_overlap_values_match_with_different_dtypes(
    ...
    store.write_dataset("baostock_cn_stock_daily_bar_unadjusted", existing, {"code": code})
    ...
    assert [item["start_date"] for item in records if item["dataset"] == "baostock_cn_stock_daily_bar_unadjusted"] == [
        "2024-01-02"
    ]
    stored = store.read_dataset("baostock_cn_stock_daily_bar_unadjusted", {"code": code})
...
def test_update_daily_adjusted_mismatch_recomputes_without_full_refetch(
    ...
    store.write_dataset("baostock_cn_stock_daily_bar_qfq", stale_qfq, {"code": code})
    ...
    assert store.read_dataset("baostock_cn_stock_daily_bar_qfq", {"code": code}).loc[0, "close"] == 8.2
...
def test_update_daily_adjusted_recompute_uses_full_local_unadjusted_history(
    ...
    store.write_dataset("baostock_cn_stock_daily_bar_unadjusted", unadjusted_history, {"code": code})
    ...
    stored = store.read_dataset("baostock_cn_stock_daily_bar_qfq", {"code": code})
    assert pd.to_datetime(stored["date"], errors="coerce").dt.strftime("%Y-%m-%d").tolist() == [
        "2024-01-01",
        "2024-01-02",
        "2024-01-03",
